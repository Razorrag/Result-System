  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full h-screen flex bg-gray-900 text-white font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <main className="flex-1 flex flex-col p-6 gap-4 h-full">
          {/* Header section */}
          <div className="flex justify-between items-center">
              <h2 className="text-3xl font-bold capitalize">
                {activeView === 'ai' ? 'AI Assistant' : 
                 activeView === 'tables' ? 'Table Viewer' : 
                 activeView === 'sql' ? 'SQL Editor' : activeView.replace(/_/, ' ')} View
              </h2>
          </div>
          ...
          <div className="flex-1 flex flex-col bg-gray-800/50 rounded-lg border border-white/10 overflow-hidden">
              ...
              {activeView === 'tables' && (
                <TableViewer data={queryResults.data} query={queryResults.query} />
              )}
              {activeView === 'sql' && (
                <SqlEditor onRunQuery={handleRunManualQuery} isLoading={isLoading} />
              )}
          </div>
        </main>
      </div>
